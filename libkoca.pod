...
Display a string in a banner fashion: chars one by one, at given speed

Usage:

	koca_banner <string> <speed>

If B<speed> is suffixed by s, or not suffixed, the string will take B<speed> seconds to be printed.

If B<speed> is suffixed by cps, B<speed> characters per second will be printed.
...
Example:
	...
	koca_banner 'Forget me not' 10cps
...
'n' is the spin type, from 0 to 3, included. 0 by default, or if n is greater than 3.
